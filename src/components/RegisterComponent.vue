...
<template>
    <h2 class="text-2xl font-bold">_REGISTEERI_</h2>
    <br>
    <form @submit.prevent="createUser">
        <div class="grid grid-cols-1 gap-6">
            <label class="block">
                <span>_Kasutaja nimi_</span>
            ...
            <label class="block">
                <span>_Korda Parooli_</span>
                <input type="password" class="mt-1 block w-full" v-model="passwordConfirm">
            </label>
        </div>
  
        <button type="submit"
            class="mt-4 text-white desktop-xl:text-2xl bg-black px-4 py-2 border-2 rounded border-black hover:bg-white dark:hover:bg-main-dark-bg hover:text-black">Tee konto</button>
    </form>
</template>
